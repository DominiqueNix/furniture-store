...
export const FilterPanel = ({filteredItems, setFilteredItems, items}) => {
    ...
    return(
        ...
        //         </Box>
              <Card sx={{ width: '100%', margin: '20px', height: '70px', display:'flex',alignItems:'center'}}>
                <Box sx={{display: 'flex', flexDirection: 'row', paddingBottom: '0', margin: '0 2rem 0 2rem'}}>
                    <FilterListIcon />
                ...
                <Autocomplete 
                    ...
                    onChange={(e, newValue) => handleFilterChange('type', newValue ? newValue.toUpperCase() : null)}
                    renderInput={(params) => <TextField {...params} label="Type"/>}
                />
                <Autocomplete 
                    id="style"
                    sx={{width: '250px', marginLeft: '20px'}}
                    onChange={(e, newValue) => handleFilterChange('style', newValue ? newValue.replace(" ", "_").toUpperCase() : null)}
                ...
                <Autocomplete 
                    id="material"
                    sx={{width: '250px', marginLeft: '20px'}}
                    onChange={(e, newValue) => handleFilterChange('material', newValue ? newValue.replace(" ", "_").toUpperCase() : null)}
                    options={utils.options.material}
                    renderInput={(params) => <TextField {...params} label="Material"/>}
                />
                <Autocomplete 
                    id="color"
                    onChange={(e, newValue) => handleFilterChange('color', newValue ? newValue.toUpperCase() : null)}
                ...
                <Autocomplete 
                    id="room"
                    sx={{width: '250px', marginLeft: '20px'}}
                    onChange={(e, newValue) => handleFilterChange('room', newValue ? newValue.replace(" ", "_").toUpperCase() : null)}
